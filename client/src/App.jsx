...
const STYLES = [
  ...
  { id: "vintage_film",  label: "Vintage Film",  icon: "📷", desc: "Retro grain, faded tones",                   sdKeywords: "vintage film photography, 35mm grain, faded kodachrome tones, retro 1970s aesthetic, analog" },
...
// ── API helpers (all go through our Express backend) ─────────────────────────
...
export default function App() {
  ...
  const analyzeAndGenerate = async () => {
    ...
    try {
      // 1 ── Claude photo analysis ──────────────────────────────────────────
      setProgressLabel("Analyzing your photo with Claude...");
      setProgress(15);

      const styleObj = STYLES.find(s => s.id === selectedStyle);
      const analysisData = await claudeRequest({
        model: "claude-sonnet-4-20250514",
        max_tokens: 800,
        messages: [{ role: "user", content: [
          { type: "image", source: { type: "base64", media_type: imageMime, data: imageBase64 } },
          { type: "text",  text: `Analyze this portrait photo and return ONLY valid JSON (no markdown, no backticks):
{"face_shape":"...","skin_tone":"...","hair":"...","notable_features":"...","current_mood":"...","composition":"...","suggestions":["tip1","tip2","tip3"]}` },
      ...
      setProgress(35);
      let analysis = {};
      try { analysis = JSON.parse((analysisData.content?.[0]?.text || "{}").replace(/```json|```/g, "").trim()); }
      catch (_) { analysis = { suggestions: [] }; }
      setAiAnalysis(analysis);

      // 2 ── Claude prompt generation ───────────────────────────────────────
      setProgressLabel("Crafting your Stable Diffusion prompt...");
      setProgress(52);

      const promptData = await claudeRequest({
        model: "claude-sonnet-4-20250514",
        max_tokens: 500,
        messages: [{ role: "user", content:
          `You are an expert Stable Diffusion img2img prompt engineer.
...
Write a single optimized img2img prompt (100-140 words). Preserve the subject's face and identity. Include: detailed subject description, lighting, color palette, camera lens, art style keywords, quality boosters like "masterpiece, best quality, highly detailed, 8k". Output the prompt only — no preamble.`
        }],
      });

      setProgress(67);
      const finalPrompt = promptData.content?.[0]?.text?.trim() || "";
      setGeneratedPrompt(finalPrompt);

      // 3 ── HuggingFace image generation ──────────────────────────────────
      setProgressLabel("Generating your aesthetic image...");
      setProgress(75);
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#0a0a0f 0%,#12071e 50%,#07111e 100%)", fontFamily: "'Segoe UI',system-ui,sans-serif", color: "#e8e0f0" }}>
      ...
      <div style={{ padding: "24px 32px 0", display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <div>
          ...
          <p style={{ margin: 0, fontSize: 12, color: "#7c6a9a" }}>Claude vision + Stable Diffusion img2img • keys secured server-side</p>
        </div>
      </div>
      ...
      <div style={{ padding: "0 32px 48px" }}>
        ...
        {step === "generating" && (
          <div style={{ textAlign: "center", padding: "80px 32px" }}>
            ...
            <h2 style={{ margin: "0 0 10px", fontSize: 26, fontWeight: 700, background: "linear-gradient(90deg,#c084fc,#f472b6)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Creating your aesthetic portrait...</h2>
            <p style={{ margin: "0 0 6px", color: "#9a8ab0", fontSize: 14 }}>{progressLabel}</p>
            <p style={{ margin: "0 0 36px", color: "#4a3a6a", fontSize: 12 }}>
              {progress > 65 ? "Image generation takes 20–60s. Cold model start may add extra time." : "Claude is analyzing your photo and building the perfect prompt..."}
            </p>
